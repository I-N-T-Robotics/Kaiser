...
class AngularRampLogger(val dvf: DriveViewerFactory) : LinearOpMode() {
    override fun runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            t.addSplit()
            val av = view.angIMU.getRobotAngularVelocity(AngleUnit.RADIANS)
            val time = t.addSplit()
...
class ForwardPushTest(val dvf: DriveViewerFactory) : LinearOpMode() {
    override fun runOpMode() {
        val view = dvf.make(hardwareMap)
...
class ForwardRampLogger(val dvf: DriveViewerFactory) : LinearOpMode() {
    override fun runOpMode() {
        val view = dvf.make(hardwareMap)
        view.setBulkCachingMode(LynxModule.BulkCachingMode.MANUAL)
...
class LateralRampLogger(val dvf: DriveViewerFactory) : LinearOpMode() {
    override fun runOpMode() {
        val view = dvf.make(hardwareMap)
        view.setBulkCachingMode(LynxModule.BulkCachingMode.MANUAL)
...
fun lateralSum(view: DriveViewer): Double {
    require(view.type == DriveType.MECANUM)
...
class LateralPushTest(val dvf: DriveViewerFactory) : LinearOpMode() {
    override fun runOpMode() {
        val view = dvf.make(hardwareMap)
...
class ManualFeedforwardTuner(val dvf: DriveViewerFactory) : LinearOpMode() {
    companion object {
        @JvmField
        var DISTANCE = 64.0
    }
...
class MecanumMotorDirectionDebugger(val dvf: DriveViewerFactory) : LinearOpMode() {
    companion object {
        @JvmField
        var MOTOR_POWER = 0.7
    }
